...
final_log_model_basic = ['hero_sum', 'cc_size_defend', 'base_ww_defend',
                         'th_attack_lower', 'th_attack_higher', 'first_attack',
                         'army_size_total']
final_log_model_basic_with_leg_trophy = ['hero_sum', 'cc_size_defend', 'base_ww_defend',
                                         'th_attack_lower', 'th_attack_higher', 'first_attack',
                                         'army_size_total', 'leg_trophy_attack']
final_log_model_users = ['hero_sum', 'cc_size_defend', 'base_ww_defend',
                         'th_attack_lower', 'th_attack_higher', 'first_attack',
                         'army_size_total', 'user_0', 'user_10001', 'user_10002',
                         'user_10003', 'user_10004', 'user_10005', 'user_10006', 'user_10007',
                         'user_10008', 'user_10010', 'user_10011', 'user_10014', 'user_10015']
final_log_model_full = ['hero_total_strength', 'cc_size_defend', 'base_ww_defend',
                        'th_attack_lower', 'th_attack_higher', 'first_attack',
                        'army_size_total', 'user_0', 'user_10001', 'user_10002',
                        'user_10003', 'user_10004', 'user_10005', 'user_10006', 'user_10007',
                        'user_10008', 'user_10010', 'user_10011', 'user_10014', 'user_10015',
                        'leg_trophy_attack', 'strat_blizzard']
model_options = [final_log_model_full]
...
scale_cols = ['th_diff', 'hero_total_strength', 'cc_size_defend', 'base_ww_defend',
              'spell_avg_below_max', 'equip_sum', 'hero_equip_strength', 'army_size_total', 
              'leg_trophy_attack', 'war_stars_defend', 'hero_sum', 'hero_num',
              'th_dist_total', 'army_tot_avg_below_max', 'spell_max_percent', 'army_max_percent',
              'leg_trophy_defend', 'war_stars_attack', 'prof_lvl_attack', 'prof_lvl_defend',
              'num_spells']
...
for list_of_vars in model_options:
    ...
    for a in lam:
        if regression_type == 'linear':
            ...
            if regularization_type == 'ridge':
                lm = LogisticRegression(penalty='l2', C=a)
            elif regularization_type == 'lasso':
                lm = LogisticRegression(penalty='l1', solver='liblinear', C=a)
            lm.fit(np.array(X_train).reshape(-1,num_predictors), y_train)
...
if (regression_type == 'logistic') & (model_final):        
    ...
    plt.hlines(bin_means, bin_edges[:-1], bin_edges[1:], colors='g', lw=2,
               label='binned empirical 3-star probability')
    ...
    plt.ylabel("3-star attack")
    plt.legend(loc = 6)
    plt.show()

# save final model outputs
if model_final:
    joblib.dump(lm, f'models/{regression_type}_{target_var}/{model_name}/model.pkl')
    model_params.to_csv(f'models/{regression_type}_{target_var}/{model_name}/details.csv', index=False)
    test_metrics.to_csv(f'models/{regression_type}_{target_var}/{model_name}/metrics.csv', index=False)
    
    if regression_type == 'logistic':
        with open(f'models/{regression_type}_{target_var}/{model_name}/scaling_mean_dict.pkl', 'wb') as f:
            pickle.dump(scaling_dict_mean, f)
        with open(f'models/{regression_type}_{target_var}/{model_name}/scaling_std_dict.pkl', 'wb') as f:
            pickle.dump(scaling_dict_std, f)
    
    df_train_pred['pred_set'] = 'train'
    df_test_pred['pred_set'] = 'test'
    all_preds = pd.concat([df_test_pred, df_train_pred], axis=0).sort_values(by='attack_id')
    all_preds.to_csv(f'models/{regression_type}_{target_var}/{model_name}/predictions.csv', index=False)
